...
import com.karumi.shot.domain.{DifferentScreenshots, Screenshot, ScreenshotNotFound, ScreenshotsComparisionResult}
...
object ShotErrorMessages {
  ...
  val diffScreenshotsModifiedMessage = "   🤔  The application UI has been modified for test: name"
  val diffOriginalScreenshotMessage = "            💾  You can find the original screenshot here: recordedPath"
  val diffRecordedScreenshotMessage = "            🆕  You can find the new recorded screenshot here: temporalPath/name"
}
...
class ConsoleReporterSpec
  ...
  private val screenShot: Screenshot = new Screenshot(screenshotName,
    "recordedPath", s"temporalPath/$screenshotName", "", "", null, "", Seq(), Seq(), Seq(), null)
  private val screenShotNotFound: ScreenshotNotFound = new ScreenshotNotFound(screenShot)
  private val differentScreenshots: DifferentScreenshots = new DifferentScreenshots(screenShot)
  private var comparisionResultWithNotFoundError: ScreenshotsComparisionResult = _
  private var comparisionResultWithDiffScreenshotsError: ScreenshotsComparisionResult = _

  before {
    consoleReporter = new ConsoleReporter(console, base64Encoder)
    comparisionResultWithNotFoundError = ScreenshotsComparisionResult(List(screenShotNotFound), Seq(screenShot))
  ...
  it should "show screenshot not found error when comparision screenshot not found and base64 is not enabled" in {
    ...
    consoleReporter.showErrors(comparision = comparisionResultWithNotFoundError,
  ...
  it should "show error messages when there are different screenshots and base64 is not enabled" in {
    ...
    consoleReporter.showErrors(comparision = comparisionResultWithDiffScreenshotsError,
      shouldPrintBase64Error = false,
      outputFolder = "")
  }
